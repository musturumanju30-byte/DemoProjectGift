    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl sm:text-3xl font-black text-gray-900 tracking-tight flex items-center gap-2.5 mt-0.5">
            <Briefcase className="h-6 w-6 text-[#F72585]" />
            <span>Corporate Inquiries</span>
          </h1>
          <p className="text-xs text-gray-500 mt-1">
            Manage custom bulk gifting leads, follow up via WhatsApp, and track deal closures.
          </p>
        </div>

        <button
          onClick={fetchInquiries}
          className="flex items-center gap-2 px-3.5 py-2 rounded-xl bg-white hover:bg-gray-50 text-xs font-semibold text-gray-700 border border-gray-200 transition self-start sm:self-auto shadow-2xs cursor-pointer"
        >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="rounded-2xl bg-white border border-gray-150 p-5 shadow-2xs space-y-1">
          <div className="text-[11px] font-bold text-gray-400 uppercase tracking-wider">Total Leads</div>
          <div className="text-2xl font-black text-gray-950">{totalCount}</div>
          <div className="text-[10px] text-gray-500">All submitted inquiries</div>
        </div>

        <div className="rounded-2xl bg-white border border-rose-200 p-5 shadow-2xs space-y-1">
          ...
          <div className="text-2xl font-black text-rose-600">{newCount} New</div>
          <div className="text-[10px] text-gray-500">Needs phone/WhatsApp call</div>
        </div>

        <div className="rounded-2xl bg-white border border-amber-200 p-5 shadow-2xs space-y-1">
          <div className="text-[11px] font-bold text-amber-700 uppercase tracking-wider">In Negotiation</div>
          <div className="text-2xl font-black text-amber-600">{inProgressCount}</div>
          <div className="text-[10px] text-gray-500">Contacted / Quoted</div>
        </div>

        <div className="rounded-2xl bg-white border border-emerald-200 p-5 shadow-2xs space-y-1">
          <div className="text-[11px] font-bold text-emerald-700 uppercase tracking-wider">Deals Closed</div>
          <div className="text-2xl font-black text-emerald-600">{closedCount}</div>
          <div className="text-[10px] text-gray-500">Successfully fulfilled</div>
        </div>
      </div>

      {/* Filters Toolbar */}
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white p-4 rounded-3xl border border-gray-150 shadow-2xs">
        ...
        <div className="relative w-full sm:w-72">
          ...
          <input
            ...
            className="w-full rounded-xl border border-gray-200 bg-gray-50/50 pl-9.5 pr-4 py-2 text-xs text-gray-900 placeholder:text-gray-400 focus:bg-white focus:border-[#F72585] focus:outline-none"
          />
        </div>
      </div>
      ...
      <div className="space-y-4">
        {isLoading ? (
          <div className="rounded-3xl bg-white border border-gray-150 p-12 text-center text-gray-400 text-xs">
          ...
          <div className="rounded-3xl bg-white border border-gray-150 p-12 text-center text-gray-400 text-xs space-y-2">
            <Briefcase className="h-8 w-8 text-gray-400 mx-auto mb-2" />
            <p className="font-bold text-gray-800">No corporate inquiries match this filter</p>
            <p className="text-[11px] text-gray-500">
              When clients submit the bulk gifting form, inquiries will appear here automatically.
            </p>
          </div>
        ) : (
          filteredInquiries.map(inq => {
            ...
            return (
              <div
                key={inq.id}
                className="rounded-3xl bg-white border border-gray-150 p-5 sm:p-6 shadow-2xs hover:shadow-xs transition space-y-4"
              >
                {/* Header bar of inquiry */}
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-gray-100">
                  <div className="flex items-center gap-3">
                    <div className="h-10 w-10 rounded-xl bg-pink-50 text-[#F72585] border border-pink-100 flex items-center justify-center font-bold shrink-0">
                      <Building2 className="h-5 w-5" />
                    </div>
                    <div>
                      <div className="flex items-center gap-2">
                        <h3 className="text-base font-black text-gray-900">{inq.companyName}</h3>
                        <span
                      ...
                      <p className="text-xs text-gray-500">
                        Contact: <strong className="text-gray-800">{inq.contactPerson}</strong>
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <select
                      ...
                      className="rounded-xl border border-gray-200 bg-gray-50 px-3 py-1.5 text-xs font-bold text-gray-800 focus:border-[#F72585] focus:outline-none cursor-pointer"
                    >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 text-xs">
                  <div className="p-3 rounded-2xl bg-gray-50/70 border border-gray-100">
                    ...
                    <div className="font-semibold text-gray-800 mt-0.5">{inq.occasion}</div>
                  </div>

                  <div className="p-3 rounded-2xl bg-gray-50/70 border border-gray-100">
                    <div className="text-[10px] uppercase font-bold text-[#C9A227]">Estimated Quantity</div>
                    <div className="font-semibold text-gray-800 mt-0.5">{inq.quantity}</div>
                  </div>

                  <div className="p-3 rounded-2xl bg-gray-50/70 border border-gray-100">
                    <div className="text-[10px] uppercase font-bold text-[#C9A227]">Phone &amp; WhatsApp</div>
                    <div className="font-semibold text-gray-800 mt-0.5 flex items-center gap-1">
                ...
                {inq.message && (
                  <div className="p-3.5 rounded-2xl bg-gray-50 border border-gray-100 text-xs text-gray-700">
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 pt-1">
                  <div className="text-[11px] text-gray-500">
                    Email: <a href={`mailto:${inq.email}`} className="text-[#F72585] hover:underline">{inq.email}</a>
                  </div>

                  <div className="flex items-center gap-2">
                    <a
                      href={`tel:${cleanPhone}`}
                      className="px-3 py-1.5 rounded-xl bg-gray-100 hover:bg-gray-200 text-gray-700 text-xs font-semibold transition flex items-center gap-1.5"
                    >
